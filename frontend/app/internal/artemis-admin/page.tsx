...
function recordToForm(r: ArtemisRecord): FormState {
  const g = (r.entityInformation?.generalDetails || {}) as Record<string, string>;
  const c = (r.entityInformation?.corporateSpecific || {}) as Record<string, string>;
  const i = (r.entityInformation?.individualSpecific || {}) as Record<string, string>;
  const s = (r.screeningAndSearchConclusion?.summary || {}) as Record<string, unknown>;
  return {
    metadata: {
      ...
      nextPeriodicReviewCycleDate: r.metadata?.nextPeriodicReviewCycleDate?.slice?.(0, 10) || '',
    },
    ...
    matchDetailsJson: JSON.stringify(r.screeningAndSearchConclusion?.matchDetails || [], null, 2),
...
export default function ArtemisAdminPage() {
  ...
  const load = useCallback(async () => {
    ...
    setError(null);
    try {
      const { data } = await internalArtemisApi.list({ q: debouncedQ || undefined, limit: 100 });
      setItems(data.items);
    } catch {
    ...
    <Box minHeight="100vh" py={3} px={2} display="flex" justifyContent="center">
        ...
        <Stack direction="row" justifyContent="space-between" alignItems="flex-start" gap={2}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              Edit Artemis records. Not linked from the main app.
            </Typography>
          </Box>
                ...
                {items.map((row) => (
                  <TableRow key={row._id} hover>
                    <TableCell sx={{ fontWeight: 600 }}>
                      {(row.entityInformation?.generalDetails?.name as string) || '—'}
                    </TableCell>
                    ...
                    <TableCell align="right">
                      ...
                      <IconButton size="small" color="error" onClick={() => setDeleteId(row._id)} aria-label="Delete">
                        <FiTrash2 />
                      </IconButton>
                    </TableCell>
                  </TableRow>
      ...
      <Dialog open={dialogOpen} onClose={() => setDialogOpen(false)} fullWidth maxWidth="md">
        ...
        <DialogContent dividers>
          ...
          {tab === 0 && (
            <Stack spacing={2} sx={{ pt: 1 }}>
              <FormControl fullWidth>
                ...
                <Select
                  ...
                  onChange={(e) =>
                    setForm((f) => ({
                      ...f,
                      metadata: { ...f.metadata, entityType: e.target.value as EntityType },
                    }))
                  }
                >
              ...
              <TextField
                ...
                onChange={(e) => setForm((f) => ({ ...f, metadata: { ...f.metadata, caseId: e.target.value } }))}
              />
              <TextField
                label="Customer ID"
                fullWidth
                value={form.metadata.customerId}
                onChange={(e) => setForm((f) => ({ ...f, metadata: { ...f.metadata, customerId: e.target.value } }))}
              />
              <TextField
                label="Case status"
                fullWidth
                value={form.metadata.caseStatus}
                onChange={(e) => setForm((f) => ({ ...f, metadata: { ...f.metadata, caseStatus: e.target.value } }))}
              />
              <TextField
                label="Approval status"
                fullWidth
                value={form.metadata.approvalStatus}
                onChange={(e) => setForm((f) => ({ ...f, metadata: { ...f.metadata, approvalStatus: e.target.value } }))}
              />
              <TextField
                label="Risk rating"
                fullWidth
                value={form.metadata.riskRating}
                onChange={(e) => setForm((f) => ({ ...f, metadata: { ...f.metadata, riskRating: e.target.value } }))}
              />
              <TextField
                ...
                onChange={(e) =>
                  setForm((f) => ({ ...f, metadata: { ...f.metadata, nextPeriodicReviewCycleDate: e.target.value } }))
                }
              />
            </Stack>
          )}
          {tab === 1 && (
            <Stack spacing={2} sx={{ pt: 1 }}>
              ...
              <TextField
                ...
                onChange={(e) => setForm((f) => ({ ...f, generalName: e.target.value }))}
              />
              <TextField
                label="Alias"
                fullWidth
                value={form.generalAlias}
                onChange={(e) => setForm((f) => ({ ...f, generalAlias: e.target.value }))}
              />
              <TextField
                label="Email"
                fullWidth
              ...
              <TextField
                ...
                onChange={(e) => setForm((f) => ({ ...f, generalPhone: e.target.value }))}
              />
              <TextField
                label="Address"
                fullWidth
                ...
                onChange={(e) => setForm((f) => ({ ...f, generalAddress: e.target.value }))}
              />
              {form.metadata.entityType === 'CORPORATE' && (
                <>
                  ...
                  <TextField
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, corpIncorporationNumber: e.target.value }))}
                  />
                  <TextField
                    label="Country of incorporation"
                    fullWidth
                    value={form.corpCountryOfIncorporation}
                    onChange={(e) => setForm((f) => ({ ...f, corpCountryOfIncorporation: e.target.value }))}
                  />
                  <TextField
                    label="Primary business activity"
                    fullWidth
                    value={form.corpPrimaryBusinessActivity}
                    onChange={(e) => setForm((f) => ({ ...f, corpPrimaryBusinessActivity: e.target.value }))}
                  />
                </>
              )}
              {form.metadata.entityType === 'INDIVIDUAL' && (
                <>
                  ...
                  <TextField
                    ...
                    onChange={(e) => setForm((f) => ({ ...f, indIdentificationNumber: e.target.value }))}
                  />
                  <TextField
                    label="Nationality"
                    fullWidth
                    value={form.indNationality}
                    onChange={(e) => setForm((f) => ({ ...f, indNationality: e.target.value }))}
                  />
                  <TextField
                    label="Country of residence"
                    fullWidth
                    value={form.indCountryOfResidence}
                    onChange={(e) => setForm((f) => ({ ...f, indCountryOfResidence: e.target.value }))}
                  />
                  <TextField
                    label="Occupation"
                    fullWidth
                    value={form.indOccupation}
                    onChange={(e) => setForm((f) => ({ ...f, indOccupation: e.target.value }))}
                  />
                </>
              )}
            </Stack>
          ...
          {tab === 2 && (
            <Stack spacing={2} sx={{ pt: 1 }}>
              <FormControlLabel control={<Switch checked={form.pep} onChange={(e) => setForm((f) => ({ ...f, pep: e.target.checked }))} />} label="PEP flag" />
              <FormControlLabel
                control={<Switch checked={form.sanctions} onChange={(e) => setForm((f) => ({ ...f, sanctions: e.target.checked }))} />}
                label="Sanctions flag"
              />
              <FormControlLabel
                control={<Switch checked={form.adverse} onChange={(e) => setForm((f) => ({ ...f, adverse: e.target.checked }))} />}
                label="Adverse news flag"
              />
              <FormControlLabel
                control={<Switch checked={form.restricted} onChange={(e) => setForm((f) => ({ ...f, restricted: e.target.checked }))} />}
                label="Own restricted list flag"
              />
              <FormControlLabel control={<Switch checked={form.noHit} onChange={(e) => setForm((f) => ({ ...f, noHit: e.target.checked }))} />} label="No-hit flag" />
              <TextField
                label="Officer name"
                fullWidth
          ...
          {tab === 3 && (
            <Stack spacing={2} sx={{ pt: 1 }}>
              <TextField
                ...
                onChange={(e) => setForm((f) => ({ ...f, matchDetailsJson: e.target.value }))}
              />
              <TextField
                ...
                minRows={6}
                value={form.riskJson}
                onChange={(e) => setForm((f) => ({ ...f, riskJson: e.target.value }))}
              />
              <TextField
                ...
                onChange={(e) => setForm((f) => ({ ...f, approvalHistoryJson: e.target.value }))}
              />
              <TextField
                ...
                onChange={(e) => setForm((f) => ({ ...f, modificationJson: e.target.value }))}
              />
            </Stack>
          )}
        </DialogContent>
        <DialogActions sx={{ px: 3, py: 2 }}>
          ...
          <Button variant="contained" onClick={save} disabled={!form.generalName.trim()}>
            Save
          </Button>
        </DialogActions>
      </Dialog>
